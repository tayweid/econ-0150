...
- Four OLS assumptions: (1) Linearity, (2) Homoskedasticity, (3) Independence, (4) Normality
...
3. **Independence**: The errors are independent from each other
...
5. **Check for independence**: (for time series) does the residual lag plot show autocorrelation?
6. **Check for normality**: are the residuals roughly bell-shaped?
...
### Looking Ahead — Part 4.4

We've covered all four OLS assumptions — linearity, homoskedasticity, independence, and normality — and the diagnostic tools to check each one. In Part 4.4, we focus specifically on the problem of timeseries: autocorrelation, why it makes standard errors unreliable, and how to fix it with differencing and growth rates. The diagnostic tools from this section — especially the residual plot — remain essential for every model we build.
