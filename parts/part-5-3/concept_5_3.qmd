...
## Does Drinking Water Hurt Your Grades?
<p class="subheader">Lets run a simple regression and see what happens.</p>
...
# Stress drives both water intake and lower scores
stress = np.random.uniform(0, 10, n)
water = 0.3 * stress + np.random.normal(0, 0.5, n)
water = np.clip(water, 0, 5)
score = 90 - 4 * stress + 2 * water + np.random.normal(0, 5, n)

df = pd.DataFrame({'water': water, 'score': score, 'stress': stress})

# Simple regression
model_simple = smf.ols('score ~ water', data=df).fit()

fig, ax = plt.subplots(figsize=(8, 5))
ax.scatter(df['water'], df['score'], alpha=0.4, color='#4C72B0', s=20)
x_line = np.linspace(df['water'].min(), df['water'].max(), 100)
y_line = model_simple.params['Intercept'] + model_simple.params['water'] * x_line
ax.plot(x_line, y_line, color='red', linewidth=2.5)
ax.set_xlabel('Daily Water Intake (liters)', fontsize=13)
...
```{python}
#| echo: false
print("Simple Model: Score ~ Water")
print(model_simple.summary().tables[1])
```

. . .

*Students who drink more water score **lower**?*

---

## What's Going On?
<p class="subheader">Stressed students drink more water AND score lower.</p>
...
ax1.scatter(df['stress'], df['water'], alpha=0.4, color='#DD8452', s=20)
ax1.set_xlabel('Stress Level', fontsize=13)
ax1.set_ylabel('Water Intake (liters)', fontsize=13)
ax1.set_title('Stress → Water', fontsize=14, weight='bold')

ax2.scatter(df['stress'], df['score'], alpha=0.4, color='#DD8452', s=20)
ax2.set_xlabel('Stress Level', fontsize=13)
ax2.set_ylabel('Exam Score', fontsize=13)
ax2.set_title('Stress → Score', fontsize=14, weight='bold')
...
*Stress is a **confounding variable**. It drives both high water intake and low scores.*

---

## Control for Stress
<p class="subheader">The coefficient on water flips sign.</p>

```{python}
#| echo: false

model_control = smf.ols('score ~ water + stress', data=df).fit()

print("Simple Model: Score ~ Water")
print(model_simple.summary().tables[1])
print("\n")
print("Controlled Model: Score ~ Water + Stress")
print(model_control.summary().tables[1])
```

. . .

*Without the control, water **looked harmful**. With the control, it's **helpful**.*

. . .

*This is what "holding constant" does. It strips out the confounding.*

---

## Pittsburgh Housing
<p class="subheader">What is the value of an additional square foot of living area?</p>
...
<p class="subheader">Lets predict price using `Living Area` and `Year Built`.</p>
...
# Load actual data
data = pd.read_csv('https://tayweid.github.io/econ-0150/data/pgh_housing_small.csv', index_col=0)
...
<p class="subheader">Predictor variables `Living Area` and `Year Built` are themsevles correlated.</p>
...
## Comparing Models
<p class="subheader">Compare coefficients with and without the control.</p>
...
## Log Interpretation
<p class="subheader">For log outcomes, coefficients represent proportional changes.</p>
...
<p class="subheader">A control variable compares two similar observations.</p>
...
## Summary
<p class="subheader">Main ideas about numerical control variables</p>
...
## Step 3: Compare Living Area and Year Built
...
sns.scatterplot(data, y='FINISHEDLIVINGAREA', x='YEARBLT', alpha=0.5)
