...
*Exercise 4.4: GDP and Unemployment.* Students work with quarterly macroeconomic data. They fit the levels model ($GDP \sim Unemployment$), the first differences model ($\Delta GDP \sim \Delta Unemployment$), and the growth rate model ($g_{GDP} \sim g_{Unemployment}$). For each specification, they check the residual lag plot for autocorrelation and compare the coefficients and p-values. The levels model will show significant autocorrelation in the residuals; the differenced and growth rate models will be much cleaner.

---

### Looking Ahead

We've built models with numerical predictors (4.1), categorical predictors (4.2), checked model assumptions (4.3), and handled the special challenges of time series data (4.4). In Part 5, we extend to the full multivariate framework — adding numerical controls (5.1), categorical controls (5.2), interaction effects (5.3), and model selection (5.4). The diagnostic tools from 4.3 and 4.4 apply to every model we'll build going forward.
